...
lookup_openai_api_key() = get(ENV, "OPENAI_KEY", "")
...
function trypost(
    ...
    body;
    n_retries::Int = 10,
    delays = ExponentialBackOff(n=n_retries, first_delay=0.5, max_delay=60.0, factor=2.0, jitter=0.1),
    retry=false,
)
    request = Base.retry(delays=delays,
                         check=(_, e) -> HTTP.RetryRequest.isrecoverable(e)) do
        HTTP.post(endpoint, headers, body, retry=retry)
    end
    return request()
end
...
function tryget(
    ...
    request = Base.retry(delays=delays,
                         check=(_, e) -> HTTP.RetryRequest.isrecoverable(e)) do
        HTTP.get(endpoint, headers, retry=retry)
    end
    return request()
end
...
function get_gpt_models(;
    ...
    # Get model list
    response = tryget(endpoint, openai_headers())
    return JSON3.read(response.body)
end
